...
public abstract class BaseRobot {
   ...
   abstract void play();
    ...
    public double heuristic(MapLocation start, MapLocation target) {
        int distance = start.distanceSquaredTo(target);
        // Add a value for the rubble at start
        try{
            distance += rc.senseRubble(start);
            distance += (rc.isLocationOccupied(start)? 100:0);
        } catch (GameActionException e){
            // If it can't sense the rubble outside it's sight range, just add nothing
        }
        return distance;
    }
