...
public class Soldier extends BaseRobot {
    ...
    public ArrayList<MapLocation> possibleLocations = new ArrayList<>();
    ...
    public void play() {
        ...
        try {
            ...
            if (turnCount == 1) {
                ...
                // Store possible archon locations in arraylist
                possibleLocations.add(new MapLocation(rc.getMapWidth()-1 - spawnX, rc.getMapHeight()-1 - spawnY));
                possibleLocations.add(new MapLocation(rc.getMapWidth()-1 - spawnX, spawnY));
                possibleLocations.add(new MapLocation(spawnX, rc.getMapHeight()-1 - spawnY));

                // Random direction to start at (random index of array)
                initialIndex = rng.nextInt(possibleLocations.size());

                //System.out.println("Soldier initial direction: "+ possibleLocations.get(initialIndex));
            }
            ...
            Arrays.sort(enemies, (o1, o2) -> { return compareAttackImportance(o1, o2); });
            ...
            if (enemies.length > 0) {
                ...
                // head towards a found archon location
                boolean moved = moveTowardsFoundArchon();

                if(moved==false){
                    if (searching) {
                        potentialArchon = possibleLocations.get(initialIndex);
                ...
                rc.setIndicatorString(""+possibleLocations.get(initialIndex));
                // if searching but current one is not valid, then switch to not searching
                if (searching && isNotOpponentArchon(potentialArchon)) {
                    possibleLocations.remove(initialIndex);
                    initialIndex %= possibleLocations.size();
                    searching = false;
                }else if (dir == null){
                    return;
    ...
    // Head towards found archon location
    public boolean moveTowardsFoundArchon() throws GameActionException{
        int value = 0;
        boolean moved = false;
        for (int i = 0; i<archonCount; i++){
    ...
    public int compareAttackImportance(RobotInfo o1, RobotInfo o2) {
